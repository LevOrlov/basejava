...
- Реализуйте класс `storage.ArrayStorage` (вставте код в пустые методы класса):
 работа с `model.Resume` на основе массива (методы `clear, get, save, delete, getAll, size`)
   - Не используйте в решении коллекции, реализацию на их основе мы добавим позднее
   - Не меняйте синтаксис методов `storage.ArrayStorage` и не меняйте класс `model.Resume`. Нужет только код внутри готовых методов `storage.ArrayStorage`
...
### Optional
- Модифицировать класс `storage.ArrayStorage`: хранить все резюме в начале storage (без дырок null), чтобы не перебирать каждый раз все 10000 элементов.
```
...
```
- Посмотреть на класс `Arrays`. Там есть полезные вещи, которые могут упростить код `storage.ArrayStorage`.
